...
int test_l2hq_funcs_internal(
    const char *pszHqSvrIP,
    short nPort,
...
int test_l2hq_funcs()
{
    ...
    cout << "\n\tL2User(" << szL2User << "), L2Password(" << "xxxxxx" << ")" << endl;
    ...
    for (int i=0; i<10; i++)
    {
        test_l2hq_funcs_internal(
            L2HostArray[nHostNum].pszHost,
    ...
    const char *pszL2Password)
{
    ...
    //连接服务器
    int nConn = TdxL2Hq_Connect(pszHqSvrIP, nPort, pszL2User, pszL2Password, Result, ErrInfo);
    if (nConn < 0)
    {
...
#if F14
    {
        ...
        bool1 = TdxL2Hq_GetSecurityQuotes10(nConn, Market, Zqdm, &ZqdmCount, Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
...
#if F15
    {
        ...
        //
        Count = 50;
        bool1 = TdxL2Hq_GetDetailTransactionData(nConn, 0, "000001", 0, &Count, Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
        ...
        while (1)
        {
          ...
          bool1 = TdxL2Hq_GetDetailTransactionData(nConn, 1, "601375", start, &count, Result, ErrInfo);
          if (!bool1)
          {
              cout << ErrInfo << endl;
              return 0;
...
#if F16
    {
        ...
        //
        Count = 100;
        bool1 = TdxL2Hq_GetDetailOrderData(nConn, 0, "000001", 0, &Count, Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
...
#if F17
    {
        ...
        //
        bool1 = TdxL2Hq_GetBuySellQueue(nConn, 1, "600300", Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
        ...
        //
        bool1 = TdxL2Hq_GetBuySellQueue(nConn, 0, "000001", Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
...
#if F1
    {
        ...
        Count = -1;
        bool1 = TdxL2Hq_GetSecurityCount(nConn, 0, &Count, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;//连接失败
            getchar();
        ...
        Count = -1;
        bool1 = TdxL2Hq_GetSecurityCount(nConn, 1, &Count, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;//连接失败
            getchar();
...
#if F2
    {
        ...
        Count = -1;
        bool1 = TdxL2Hq_GetSecurityList(nConn, 0, 0, &Count, Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;//连接失败
            getchar();
...
#if F3
    {
        ...
        short ZqdmCount = 4;
        bool1 = TdxL2Hq_GetSecurityQuotes(nConn, xMarket, Zqdm, &ZqdmCount, Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
...
#if F4ALL
    {
        ...
        for (int i=0; i<sizeof(szCategoryInfo)/sizeof(szCategoryInfo[0]); i++)
        {
            ...
            bool1 = TdxL2Hq_GetSecurityBars(nConn, i, 1, "600600", 0, &Count, Result, ErrInfo);
            if (!bool1)
            {
                cout << ErrInfo << endl;
                return 0;
        ...
        for (int i=0; i<sizeof(szCategoryInfo)/sizeof(szCategoryInfo[0]); i++)
        {
            ...
            bool1 = TdxL2Hq_GetSecurityBars(nConn, i, 0, "000001", 0, &Count, Result, ErrInfo);
            if (!bool1)
            {
                cout << ErrInfo << endl;
                return 0;
...
#if F5ALL
    {
        ...
        for (int i=0; i<sizeof(szCategoryInfo)/sizeof(szCategoryInfo[0]); i++)
        {
            ...
            bool1 = TdxL2Hq_GetIndexBars(nConn, i, 1, "000001", 0, &Count, Result, ErrInfo);
            if (!bool1)
            {
                cout << ErrInfo << endl;
                return 0;
        ...
        for (int i=0; i<sizeof(szCategoryInfo)/sizeof(szCategoryInfo[0]); i++)
        {
            ...
            bool1 = TdxL2Hq_GetIndexBars(nConn, i, 0, "399001", 0, &Count, Result, ErrInfo);
            if (!bool1)
            {
                cout << ErrInfo << endl;
                return 0;
...
#if F6
    {
        ...
        //获取分时图数据
        bool1 = TdxL2Hq_GetMinuteTimeData(nConn, 0, "000001",  Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
...
#if F7
    {
        ...
        //获取历史分时图数据
        bool1 = TdxL2Hq_GetHistoryMinuteTimeData(nConn, 0, "000001", 20170112, Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
...
#if F8
    {
        ...
        //获取分笔图数据
        Count = 20;
        bool1 = TdxL2Hq_GetTransactionData(nConn, 0, "000001", 0, &Count, Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
...
#if F9
    {
        ...
        //获取历史分笔图数据
        Count = 100;
        bool1 = TdxL2Hq_GetHistoryTransactionData(nConn, 0, "000001", 0, &Count, 20170112,  Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
...
#if F10
    {
        ...
        //获取F10数据的类别
        bool1 = TdxL2Hq_GetCompanyInfoCategory(nConn, 0, "000001", Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
...
#if F11
    {
        ...
        //获取F10数据的某类别的内容
        bool1 = TdxL2Hq_GetCompanyInfoContent(nConn, 1, "600030", "600030.txt", 142577, 5211, Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
...
#if F12
    {
        ...
        //获取除权除息信息
        bool1 = TdxL2Hq_GetXDXRInfo(nConn, 0, "000001", Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
...
#if F13
    {
        ...
        //获取财务信息
        bool1 = TdxL2Hq_GetFinanceInfo(nConn, 0, "000001", Result, ErrInfo);
        if (!bool1)
        {
            cout << ErrInfo << endl;
            return 0;
